...
def calculate_cluster_embeddings(item_embeddings, cluster_to_items, item_token_map):
    ...
    for cluster_id, item_list in cluster_to_items.items():
        ...
        for item_id in item_list:
            # Try to find the embedding using the token mapping
            if item_id in item_token_map:
                # Get the token number
                token = item_token_map[item_id]
                # Convert to string (embeddings use string keys)
                token_str = str(token)
                
                if token_str in item_embeddings:
                    embedding = item_embeddings[token_str]
        ...
        if missing_items and len(missing_items) <= 10:
            ...
            print(f"  Sample missing: {missing_items[:5]}")
        elif missing_items:
            print(f"  Warning: {len(missing_items)} items not found in embeddings")
...
def save_cluster_embeddings(cluster_embeddings):
    ...
    # Also save as numpy array for easy access (only if we have all 5 clusters)
    if len(cluster_embeddings) == 5:
        cluster_array = np.array([cluster_embeddings[str(i)] for i in range(5)])
...
def analyze_cluster_embeddings(cluster_embeddings):
    ...
    embeddings_array = np.array([cluster_embeddings[str(i)] for i in range(5)])
